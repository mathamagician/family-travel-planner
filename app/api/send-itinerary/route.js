...
export async function POST(request) {
  try {
    const { email, destination, days, pdfBase64 } = await request.json();
    ...
    const dayRows = days.map(day => {
      const slots = (day.slots ?? [])
        ...
        .map(s => {
          ...
          const dur = s.duration_mins ? `${s.duration_mins}m` : "";
          ...
          return `<tr>
            ...
            <td style="padding:6px 10px;border-bottom:1px solid #F0EDE8;">
              ...
              <span style="font-size:11px;color:#8A9BA5;margin-left:6px;">${dur}${loc}</span>
              ${notes}${desc}
            </td>
          </tr>`;
        }).join("");
