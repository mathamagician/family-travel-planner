...
const SYSTEM_PROMPT = "You are a family travel expert. When given a city, children's ages, and trip length, generate 12-16 activities. CRITICAL: Respond with ONLY a valid JSON array. Each object: {\"id\":\"snake_id\",\"name\":\"Name\",\"type\":\"attraction\"|\"park\"|\"outdoors\"|\"culture\"|\"museum\"|\"food\"|\"entertainment\",\"hours\":\"human readable hours\",\"notes\":\"tips\",\"location\":\"address\",\"age_range\":\"min-max\",\"duration_mins\":number,\"affiliate\":\"url\"}. Rules: real places only, stroller-friendly, practical tips, mix indoor/outdoor, realistic durations, real URLs.";
...
function ActivitiesStep({profile,activities,setActivities,selectedIds,setSelectedIds,onNext,onBack}){
  ...
  const generate=async()=>{setLoading(true);setError(null);try{
    const r=await fetch("https://api.anthropic.com/v1/messages",{method:"POST",headers:{"Content-Type":"application/json"},body:JSON.stringify({model:"claude-sonnet-4-20250514",max_tokens:6000,system:SYSTEM_PROMPT,messages:[{role:"user",content:"Generate 12-16 family activities for: "+profile.destination+". Children ages: "+profile.kids.map(k=>k.age).join(", ")+". Trip: "+profile.trip_length_days+" days. ONLY JSON array."}]})});
    const d=await r.json(),txt=d.content.filter(b=>b.type==="text").map(b=>b.text).join(""),parsed=JSON.parse(txt.replace(/```json|```/g,"").trim());
    setActivities(parsed);setSelectedIds(new Set(parsed.map(a=>a.id)));
  }catch(e){console.error(e);setError("API failed — using sample data.");if(!activities.length){setActivities(SAMPLE_ACTIVITIES);setSelectedIds(new Set(SAMPLE_ACTIVITIES.map(a=>a.id)));}}finally{setLoading(false);}};
...
function ItineraryStep({profile,activities,selectedIds,onBack,onBackToActivities}){
  ...
  function onSlotDrop(e,di,si){
    ...
    // If from slot, clear source (or swap)
    if(dragSrc.type==="slot"){
      ...
      // From sidebar onto occupied — don't replace
