...
export default function ActivitiesModule({ profile, activities, setActivities, selectedIds, setSelectedIds, onNext, onSkipRestaurants, onBack, destPageBanner, onDismissBanner, preselectedNames, clearPreselect }) {
  ...
  const NavRow = ({ compact }) => (
    <div style={{ display: "flex", flexWrap: "wrap", alignItems: "center", justifyContent: "center", gap: 8, maxWidth: 1060, margin: compact ? "16px auto 10px" : "0 auto 14px" }}>
      ...
      {!compact && onSkipRestaurants && <button onClick={onSkipRestaurants} disabled={!selectedIds.size} style={{ padding: "7px 14px", borderRadius: 9, border: "2px solid var(--ocean)", background: "var(--cloud)", color: "var(--ocean)", fontSize: 10, fontWeight: 700, cursor: selectedIds.size ? "pointer" : "not-allowed", flexShrink: 0 }}>Itinerary (skip restaurants) →</button>}
    </div>
  );
  ...
  return (
    <div className="step-enter">
      ...
      <div style={{ display: "flex", justifyContent: "center", gap: 8, marginTop: 20 }}>
        ...
        {onSkipRestaurants && <button onClick={onSkipRestaurants} disabled={!selectedIds.size} style={{ padding: "10px 20px", borderRadius: 9, border: "2px solid var(--ocean)", background: "var(--cloud)", color: "var(--ocean)", fontSize: 11, fontWeight: 700, cursor: selectedIds.size ? "pointer" : "not-allowed" }}>Itinerary (skip restaurants) →</button>}
      </div>
    </div>
  );
}
